...
CREATE TABLE orders(
  id SERIAL PRIMARY KEY,
  status VARCHAR(50),
  user_id INT REFERENCES users(id) ON DELETE CASCADE ON UPDATE CASCDE
);
```
...
CREATE TABLE order_products(
  id SERIAL PRIMARY KEY,
  order_id INT REFERENCES orders(id) NOT NULL ON DELETE CASCADE ON UPDATE CASCDE,
  product_id INT REFERENCES products(id) NOT NULL  ON DELETE CASCADE ON UPDATE CASCDE,
  quantity INT
);
```
